...
(function ($) {
    $.fn.EasyTree = function (options) {
        var defaults = {
            ...
            addable: false,
            i18n: {
                deleteNull: '削除するノードを選択します',
                deleteConfirmation: 'この枠を削除しますか？',
                confirmButtonLabel: 'よし',
                editNull: '編集するノードを選択します',
        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    $(this).find(' > span > span').addClass('fa-minus-circle');
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar').append('<div class="create"><button class="btn btn-default btn-sm btn-success"><span class="fas fa-plus"></span></button></div> ');
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var item = $('<li><span><span class="fas fa-check"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        ...
                        if (selected.length <= 0) {
                            ...
                            var table = 'product_info_category';
                            var parent_id = 0;

                            addNode(table,$(createInput).find('input').val(), parent_id);
                            ...
                            if($(selected).attr('table')=='product_info_model'){
                                $(easyTree).find('.alert .alert-content').text(options.i18n.addMultiple);
                              ...
                              if ($(selected).hasClass('parent_li')) {
                                  ...
                                  $(selected).addClass('parent_li').find(' > span > span').addClass('fa-minus-circle').removeClass('fa-check');
                                  $(selected).append($('<ul></ul>')).find(' > ul').append(item);
                              }


                              var parent_table = $(selected).attr('table');
                              var table = $(selected).attr('child-table');
                              var parent_id = $(selected).attr('id');
            ...
            // editable
            if (options.editable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="edit"><button class="btn btn-default btn-sm btn-primary disabled"><span class="fas fa-edit"></span></button></div> ');
                        ...
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {
                                if ($(editor).val() !== '') {
                                    ...
                                    editNode(parent_table,$(editor).val(), parent_id, parent_id);
                                }
                            }
                        });
            ...
            // deletable
            if (options.deletable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="remove"><button class="btn btn-default btn-sm btn-danger disabled"><span class="fas fa-trash"></span></button></div> ');
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                            ...
                            if($(selected).parent('ul').find(' > li').length <= 1) {
                                $(selected).parents('li').removeClass('parent_li').find(' > span > span').removeClass('fa-minus-circle').addClass('fa-check');
                            ...
                            deleteNode(parent_table, parent_id);
                        });
...
function addNode(table, name, parent_id){
  $.post(site_url+'admin/products/add_info', {table: table, name: name, parent_id: parent_id})
              .done(function (data) {
                 location.reload();
              });
}

function editNode(table,name, parent_id, id){
  $.post(site_url+'admin/products/edit_info', {table: table, name: name, parent_id: parent_id, id:id})
              .done(function (data) {
                 location.reload();
              });
}
...
function deleteNode(table, id){
  $.post(site_url+'admin/products/delete_info', {table: table, id: id})
              .done(function (data) {
                 location.reload();
              });
}
